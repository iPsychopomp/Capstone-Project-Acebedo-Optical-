Public Class addSupplierItemsvb
    ...
    Public Sub SaveSupplierItem()
        Try
            ...
            ' Save to tbl_supplier_products database
            Call dbConn()

            Dim query As String = "INSERT INTO tbl_supplier_products (product_name, category, product_price, description, supplierID, status) VALUES (?, ?, ?, ?, ?, ?)"
            Dim cmd As New Odbc.OdbcCommand(query, conn)
            ' Use explicit parameter types for MySQL ODBC driver compatibility
            Dim p1 = cmd.Parameters.Add("@p1", Odbc.OdbcType.VarChar)
            p1.Value = productName
            Dim p2 = cmd.Parameters.Add("@p2", Odbc.OdbcType.VarChar)
            p2.Value = category
            Dim p3 = cmd.Parameters.Add("@p3", Odbc.OdbcType.Double)
            p3.Value = CDbl(productPrice)
            Dim p4 = cmd.Parameters.Add("@p4", Odbc.OdbcType.VarChar)
            p4.Value = description
            Dim p5 = cmd.Parameters.Add("@p5", Odbc.OdbcType.Int)
            If supplierID > 0 Then
                p5.Value = supplierID
            Else
                p5.Value = DBNull.Value
            End If
            Dim p6 = cmd.Parameters.Add("@p6", Odbc.OdbcType.VarChar)
            p6.Value = "Active"  ' status - set to Active by default
